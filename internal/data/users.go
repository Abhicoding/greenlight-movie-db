...
const (
	duplicateEmailConstraint = `pq: duplicate key value violates unique constraint "users_email_key"`
)
...
func (m UserModel) Insert(user *User) error {
	...
	if err != nil {
		switch {
		case err.Error() == duplicateEmailConstraint:
...
func (m UserModel) GetForToken(tokenScope, tokenPlaintext string) (*User, error) {
	...
	query := `SELECT users.id, users.created_at, users.name, users.email, users.password_hash, users.activated, users.version FROM users INNER JOIN tokens
	ON users.id = tokens.user_id WHERE tokens.hash = $1
	AND tokens.scope = $2
	AND tokens.expiry > $3`
